    ...
    return (
      <div className="max-w-md mx-auto py-12">
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-lg space-y-6 text-center">
          ...
          <div className="p-3.5 rounded-xl bg-blue-50 border border-blue-200 text-xs text-blue-900 text-left">
            <span className="font-bold block mb-1">Demo Password:</span>
            Use <code className="bg-white px-2 py-0.5 rounded border border-blue-300 font-bold text-blue-700">admin123</code> to sign in.
          </div>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4 text-left">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-slate-900 text-sm focus:outline-hidden focus:ring-2 focus:ring-blue-500"
              />
            </div>
    ...
    <div className="space-y-10">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Connected to persistent backend JSON storage (<code className="bg-slate-100 px-1 py-0.5 rounded text-blue-700">data/messages.json</code>)
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 text-xs font-semibold transition-colors cursor-pointer"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-2">
          ...
          <div className="text-[11px] text-slate-400">Stored in persistent JSON</div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-2">
          ...
          <div className="text-3xl font-extrabold text-amber-600">{unread}</div>
          <div className="text-[11px] text-slate-400">Requires response or review</div>
        </div>

        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Reviewed / Read</span>
            <div className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
              <CheckCircle2 className="w-4 h-4" />
            </div>
          </div>
          <div className="text-3xl font-extrabold text-emerald-600">{read}</div>
          <div className="text-[11px] text-slate-400">Marked as completed</div>
        </div>

        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Response Health</span>
          ...
          <div className="text-[11px] text-slate-400">Clearance rate</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-900">Submissions by Category</h2>
            </div>
        ...
        <div className="lg:col-span-4 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-4 flex flex-col justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Read vs. Unread</h2>
            <p className="text-xs text-slate-500">Inquiry status balance</p>
          </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
        {/* Table Controls */}
        <div className="p-6 border-b border-slate-200 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-300 text-xs focus:outline-hidden focus:ring-2 focus:ring-blue-500"
            />
          </div>
            ...
            <tbody className="divide-y divide-slate-200 text-slate-700">
              {filteredMessages.length > 0 ? (
                filteredMessages.map((msg) => (
                  <tr 
                    key={msg.id} 
                    className={`hover:bg-slate-50 transition-colors ${!msg.isRead ? 'bg-blue-50/40 font-medium' : ''}`}
                  >
                    ...
                    <td className="py-4 px-6 max-w-xs">
                      <div className="truncate font-semibold text-slate-800">{msg.subject}</div>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-xl font-bold text-slate-900 mt-2">{inspectedMessage.subject}</h3>
              </div>
              <button
                onClick={() => setInspectedMessage(null)}
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-1.5">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900">{inspectedMessage.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Email:</span>
                <span className="font-mono text-blue-600">{inspectedMessage.email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Received:</span>
                <span className="text-slate-700">{new Date(inspectedMessage.createdAt).toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="pt-2 flex items-center justify-between">
              ...
              <button
                ...
                className="px-3 py-2 rounded-xl text-rose-600 hover:bg-rose-50 text-xs font-semibold transition-colors"
              >
                Delete Message
              </button>
            </div>
